            ...
            <Modal v-model="addModel" title="新增" class-name="vertical-center-modal" @on-ok="addOk" :loading="loadingAdd" :mask-closable="false">
                ...
                <Form :model="drugSdccForm" ref="drugSdccFormAdd" :rules="ruleValidate" :label-width="110">
                    <FormItem label="父级SDCC" prop="superiorSdcc">
                        <Select  v-model="drugSdccForm.superiorSdcc" @on-change='getDataLevel' filterable>
                            ...
                            <OptionGroup label="四级编码">
                                <Option v-for="item in getDrugSdccLevel(3)" :value="item.sdcc" :key="item.sdcc">{{ '&nbsp;&nbsp;&nbsp;' + item.sdccName }}</Option>
                            </OptionGroup>
                        </Select>
                    </FormItem>
...
export default {
  ...
  methods: {
    ...
    getDataLevel(value) {
      if (value != "" && value != null) {
          var item = this.drugSdccAllList.filter(x => x.sdcc == value);
