<template>
    ...
    <div>
        <div v-for="(item,dataIndex) in data.backList" :key="item.errorValue">
            ...
            <div style="display:inline-block;width:90%">
                <Row class="expand-row" v-for="(rowData,rowIndex) in item.dataList" :key="rowData.id">
                    <Col span="1" v-for="(colData,index) in rowData" :key="colData">
                    <span v-if="index!='id'" class="expand-value">{{colData}}</span>
                    </Col>
                    <Col span="2">
                    <Button type="primary" @click="batchOperation(data.hospitalNum,rowData.id,rowIndex,dataIndex)">创建医院规则</Button>
                    <Button type="primary" @click="batchQuestion(rowData.id,rowIndex,dataIndex)">创建奥术大师多通用规则</Button>
                    </Col>
                </Row>
            </div>
