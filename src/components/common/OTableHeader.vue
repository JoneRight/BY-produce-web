<template>
    <table cellspacing="0" cellpadding="0" border="0" :style="styles">
                            ...
                            <Poptip
                                ...
                                <div slot="content" :class="[prefixCls + '-filter-list']" v-if="getColumn(rowIndex, index)._filterMultiple">
                                    ...
                                    <div :class="[prefixCls + '-filter-footer']">
                                        ...
                                        <i-button type="text" size="small" @click.native="handleCustomFilter(rowIndex, index)">自定义</i-button>
        ...
        <o-filter-modal ref="customFilterModal" :width="520" @ok="handleCustomFilterModalOk"/>
    </table>
</template>
<script>
    ...
    export default {
        name: 'TableHead',
        mixins: [ Mixin, Locale ],
        components: { CheckboxGroup, Checkbox, Poptip, iButton, renderHeader, OFilterModal },
        props: {
            prefixCls: String,
            styleObject: Object,
            columns: Array,
        ...
        methods: {
            ...
            handleFilter (index) {
                /** 扩展追加自定义筛选 开始 */
                if (this.$parent.cloneColumns[index]._customFilter) {
                    this.$parent.cloneColumns[index].filterMethod = this.$parent.cloneColumns[index]._customFilter.filterMethod || this.$parent.cloneColumns[idx].filterMethod
                    this.$parent.cloneColumns[index]._customFilter = {}
                }
                /** 扩展追加自定义筛选 结束 */
            ...
            /** 扩展追加自定义筛选 开始 */
            handleCustomFilter (rowIndex, index) {
                let cfg = this.getColumn(rowIndex, index)
                this.$refs.customFilterModal.show({ rowIndex, index }, cfg)
            },
            handleCustomFilterModalOk (info) {
                let { index = {}, rules = {} } = info
                let cfg = this.getColumn(index.rowIndex, index.index)
                let idx = cfg._index
                this.handleReset(idx)// 先还原
                this.$parent.cloneColumns[idx]._customFilter = { ...rules, filterMethod: cfg.filterMethod }// 备份旧方法
                this.$parent.cloneColumns[idx].filterMethod = rules.filter
                this.$parent.cloneColumns[idx]._filterChecked = ['customFilter']
                this.$parent.handleFilter(idx)
            }
            /** 扩展追加自定义筛选 结束 */
        }
    };
