<template>
    <div class="bg-white p20  m20">
        ...
        <div class="mt20">
            <Form :label-width="150" :model="UserProduct" ref="ProductForm" :rules="ruleValidate">
            
                <FormItem label="用户名称" >
                    <Input :value="userName" style="width:500px" readonly/>
                </FormItem>
                <FormItem label="产品名称" >
                    <Input v-model="Product.productName"  style="width:500px" readonly/>
                </FormItem>
                <FormItem label="用户产品名称" prop="userProductName">
                    <Input v-model="UserProduct.userProductName" style="width:500px" placeholder="请填写用户产品名称"/>
                </FormItem>
                <FormItem label="总数量" prop="totalCount">
                    <InputNumber v-model="UserProduct.totalCount" style="width:500px" :min="0" :max="999999999"/>
                </FormItem>
                <FormItem label="有效期">
                    <FormItem prop="productBeginDate" style="display:inline-block">
                        <DatePicker placeholder="请选择开始日期" format="yyyy年MM月dd日" v-model="UserProduct.productBeginDate"  type="date"   style="width: 230px;" :editable="false"></DatePicker>
                    </FormItem>
                    <span class="pl10 pr10">至</span>
                    <FormItem prop="productEndDate" style="display:inline-block">
                        <DatePicker placeholder="请选择结束日期" format="yyyy年MM月dd日" v-model="UserProduct.productEndDate"  type="date"   style="width: 230px;" :editable="false"></DatePicker>
                    </FormItem>
                </FormItem>
                <!-- 产品维度 维度值 -->
    ...
    export default {
        ...
        data () {
            const validateEndDate = (rule, value, callback) => {
                if(this.UserProduct.productBeginDate && this.UserProduct.productEndDate){
                    if(new Date(this.UserProduct.productEndDate)<new Date(this.UserProduct.productBeginDate)){
            ...
            editUserProduct() {
                ...
                self.$refs['ProductForm'].validate((valid) => {
                    ...
                    if (valid && isSelectedDimValsFlag && self.checkedMeasureData.length > 0) {
                        Vue.ajax.send({
                            name: 'user.editUserProduct',
                            data: {
                                userProductId: self.userProductId,
                                userProductName:self.UserProduct.userProductName,
                                totalCount:self.UserProduct.totalCount,
                                productBeginDate: self.UserProduct.productBeginDate.format('YYYY-MM-DD'),
                                productEndDate: self.UserProduct.productEndDate.format('YYYY-MM-DD'),
                            ...
                            success: function(data) {
                                if (!!data && data.code=='0') {
                                    ...
                                    self.$Message.success('编辑用户产品信息提交成功！');
                                } else {
                                    self.$Modal.error({
                                        title: '',
                                        content: data.message
