<template>
    <div class="bg-white p20  m20">
        <h2 class="common-title">编辑数据包</h2>
        <div class="mt20">
            <Form :label-width="100" :model="Product" ref="ProductForm" :rules="ruleValidate">
                ...
                <FormItem>
                    ...
                    <Button type="primary" @click="editProInfo">保存</Button>
                </FormItem>
            </Form>
        </div>        
    </div>
...
import Vue from "vue";
    export default {
        ...
        mounted () {
            ...
            // 获取产品信息
            this.Product.productId = this.$route.query.Id;
            this.getProductInfo();
        },
        methods: {
            // 获取所有维度 
                ...
                Vue.ajax.send({
                    ...
                    success: function(data) {
                        if (!!data && data.code == '0') {
                            ...
                            // console.error("111111111111111111111111111",data.data.productJiontSql);
                            self.getProductCubeDesc (self.Product.productJiontSql)
                            self.getMeasureList(self.Product.productJiontSql);
                            //回显此产品的维度
                            self.productDimensionTreeData = data.data.dimensionList;
                            //回显度量
                            var proMeasureIds = [];
                            data.data.measureList.forEach(measure =>{
                                proMeasureIds.push(measure.id);
                            });
                            self.checkedMeasureData = proMeasureIds;
                        } else {
                            this.$Modal.error({
                                title: '',
                                content: data.message
                        ...
                        Vue.ajax.send({
                            ...
                            success: function(data) {
                                if (!!data && data.code=='0') {
                                    ...
                                    self.$Message.success('编辑数据包信息提交成功！');
                                } else {
                                    self.$Modal.error({
                                        title: '',
                                        content: data.message
