    ...
    <div class="m15" style="height:calc(100% - 60px);overflow-y:hidden;">
        ...
        <div class="search-con">
            <Form :model="searchForm" ref="formInline" inline>
                ...
                <Button v-authorize="'RulesDuplicateAdd'" type="primary" @click="addData()" style="float:right">+ 新增</Button>
            </Form>
        </div>

        <div class="bg-white mt15" style="height:100%;">
            <CustomTable v-authorize="'RulesDuplicateList'" :columnConfig='dataCol' :columnDatas='dataList' :showModal="showModal" @onSort="sortColumn" @onOutput="showData" @onOk="showModal=false" @onCancel="showModal=false">
                <tr v-for="(item,index) in showDatas" :key="index">
                    <td v-for="(colData,colIndex) in dataCol" :key="colIndex" class="ellipsis">
                        <!-- {{item[colData.name]}} -->
                        <span v-if="colData.name=='op'">
                            <Button v-authorize="'RulesDuplicateAudit'" v-if="item['state']==1" icon="md-checkmark-circle" size="small" title="通过" type="success" @click='ruleTongguo(index)'></Button>
                            <Button v-authorize="'RulesDuplicateAudit'" v-if="item['state']==1" icon="md-close-circle" size="small" title="驳回" type="error" @click='ruleBohui(index)'></Button>
                            <Button v-authorize="'RulesDuplicateDisable'" v-if="item['state']==2" icon="md-remove" size="small" title="停用" type="error" @click='ruleTingyong(index)'></Button>
                            <Button v-authorize="'RulesDuplicateEnable'" v-if="item['state']==0" icon="md-play" size="small" title="启用" type="success" @click='ruleQiyong(index)'></Button>
                            <Button v-authorize="'RulesDuplicateEdit'" icon="md-create" size="small" title="编辑" type="primary" @click='ruleBianji(index)'></Button>
                            <Button v-authorize="'RulesDuplicateDel'" icon="md-close" size="small" title="删除" type="error" @click="ruleShanchu(index)"></Button>
                        </span>
                        <CheckboxGroup v-else-if="colData.name=='sel'" v-model="selectList" style="height:100%;">
                            <Checkbox size='small' :key="index" :value="JSON.stringify(item['checkRepeatId'])" @change="clickMe(index)"></Checkbox>
                        </CheckboxGroup>
            ...
            <div style="" class="page-footer">
                <div class="mt15 text-center" v-if="dataList!=null && dataList.length>0">
                    ...
                    <template v-if="oldStatus=='1'">
                        <Button v-authorize="'RulesDuplicateAudit'" type="primary" @click="yesAuditBatch()" :disabled="selectList.length==0" class="inputCheck" style="left: 270px;margin-top: -10px;">通过</Button>
                        <Button v-authorize="'RulesDuplicateAudit'" type="primary" @click="NoAuditBatch()" :disabled="selectList.length==0" class="inputCheck" style="left: 335px;margin-top: -10px;">驳回</Button>
                    </template>
...
export default {
    ...
    methods: {
        ...
        updateState(state) {
            var self = this;
            Vue.ajax.send({
                name: "duplicateCheck.updataState",
                data: {
                    checkRepeatId: self.commonId,
                    state: state
                },
                success: function(data) {
                    if (!!data && data.code == "0") {
        ...
        auditBatch(state) {
            if (this.selectList.length < 1) {
                ...
                this.selectList.forEach(element => {
                    idsList.push(element.checkRepeatId);
                });
        ...
        updateAudit(idsList, state) {
            var self = this;
            Vue.ajax.send({
                name: "duplicateCheck.updateAudit",
                data: {
                    idsList: idsList,
                    state: state,
        ...
        clickMe(index) {
            ...
            var dataIndex = this.selectList.indexOf(
                this.dataList[index].checkRepeatId+""
            );
            //存在就删除，不存在就添加
            if (dataIndex > -1) {
                ...
                this.selectList.push(this.dataList[index].checkRepeatId);
            }
        },
        //全选、非全选
        allClickMe(value){
            if(value==1){
                this.selectList=this.showDatas.map(a=>JSON.stringify(a['checkRepeatId']));
            }
            else{
                this.selectList=[];
            }
        ...
        ruleTongguo(index) {
            this.commonId = this.showDatas[index].checkRepeatId;
            this.auditSingle(2);
        },
        ruleBohui(index) {
            this.buTongGuo = true;
        ...
        ruleBianji(index) {
            this.editData(this.showDatas[index].checkRepeatId);
        },
        ruleShanchu(index) {
            this.delRule = true;
            this.commonId = this.showDatas[index].checkRepeatId; //当前行id
        }
    }
};
</script>
