    ...
    <div class="m15" style="height:calc(100% - 60px);overflow-y:hidden;">
        ...
        <div class="search-con">
            <Form :model="searchForm" ref="formInline" inline>
                ...
                <Button v-authorize="'RulesSpecialAdd'" type="primary" @click="addData()" style="float:right">+ 新增</Button>
            </Form>
        </div>

        <div class="bg-white mt15" style="height:100%;">
            <CustomTable v-authorize="'RulesSpecialList'" :columnConfig='dataCol' :columnDatas='dataList' :showModal="showModal" @onSort="sortColumn" @onOutput="showData" @onOk="showModal=false" @onCancel="showModal=false">
                <tr v-for="(item,index) in showDatas" :key="index">
                    <td v-for="(colData,colIndex) in dataCol" :key="colIndex" class="ellipsis">
                        <!-- {{item[colData.name]}} -->
                        <span v-if="colData.name=='op'">
                            <Button v-authorize="'RulesSpecialAudit'" v-if="item['state']==1" icon="md-checkmark-circle" size="small" title="通过" type="success" @click='ruleTongguo(index)'></Button>
                            <Button v-authorize="'RulesSpecialAudit'" v-if="item['state']==1" icon="md-close-circle" size="small" title="驳回" type="error" @click='ruleBohui(index)'></Button>
                            <Button v-authorize="'RulesSpecialDisable'" v-if="item['state']==2" icon="md-remove" size="small" title="停用" type="error" @click='ruleTingyong(index)'></Button>
                            <Button v-authorize="'RulesSpecialEnable'" v-if="item['state']==0" icon="md-play" size="small" title="启用" type="success" @click='ruleQiyong(index)'></Button>
                            <Button v-authorize="'RulesSpecialEdit'" icon="md-create" size="small" title="编辑" type="primary" @click='ruleBianji(index)'></Button>
                            <Button v-authorize="'RulesSpecialDel'" icon="md-close" size="small" title="删除" type="error" @click="ruleShanchu(index)"></Button>
                        </span>
                        <CheckboxGroup v-else-if="colData.name=='sel'" v-model="selectList" style="height:100%;">
                            <Checkbox size='small' :key="index" :value="JSON.stringify(item['specialSymbolId'])" @change="clickMe(index)"></Checkbox>
                        </CheckboxGroup>
                        ...
                        <span v-else-if="colData.name=='old_symbol'">
                            {{item['old_symbol']}}->{{item['new_symbol']}}
                        </span>
                        <span v-else>
                            {{item[colData.name]}}
                        </span>
            ...
            <div style="" class="page-footer">
                <div class="mt15 text-center" v-if="dataList!=null && dataList.length>0">
                    ...
                    <template v-if="oldStatus=='1'">
                        <Button v-authorize="'RulesSpecialAudit'" type="primary" @click="yesAuditBatch()" :disabled="selectList.length==0" class="inputCheck" style="left: 270px;margin-top: -10px;">通过</Button>
                        <Button v-authorize="'RulesSpecialAudit'" type="primary" @click="NoAuditBatch()" :disabled="selectList.length==0" class="inputCheck" style="left: 335px;margin-top: -10px;">驳回</Button>
                    </template>
...
export default {
        ...
        return {
            ...
            dataCol: [
                ...
                },
                {
                    ...
                    render: params => {
                        return;
                        params.row.old_symbol + "->" + params.row.new_symbol;
                    }
                },
                {
                    text: "状态",
    ...
    methods: {
        ...
        updateState(state) {
            var self = this;
            Vue.ajax.send({
                name: "specialChart.updataState",
                data: {
                    specialSymbolId: self.commonId,
                    state: state
                },
                success: function(data) {
                    if (!!data && data.code == "0") {
        ...
        auditBatch(state) {
            if (this.selectList.length < 1) {
                ...
                this.selectList.forEach(element => {
                    idsList.push(element.specialSymbolId);
                });
        ...
        clickMe(index) {
            ...
            var dataIndex = this.selectList.indexOf(
                this.showDatas[index].specialSymbolId + ""
            );
            //存在就删除，不存在就添加
            if (dataIndex > -1) {
                this.selectList.splice(dataIndex, 1);
            } else {
                this.selectList.push(this.showDatas[index].specialSymbolId);
            }
        },
        //全选、非全选
        allClickMe(value) {
            if (value == 1) {
                this.selectList = this.showDatas.map(a =>
                    JSON.stringify(a["specialSymbolId"])
                );
            } else {
                this.selectList = [];
            }
        ...
        ruleTongguo(index) {
            this.commonId = this.showDatas[index].specialSymbolId;
            this.auditSingle(2);
        },
        ruleBohui(index) {
            this.buTongGuo = true;
        ...
        ruleBianji(index) {
            this.editData(this.showDatas[index].specialSymbolId);
        },
        ruleShanchu(index) {
            this.delRule = true;
            this.commonId = this.showDatas[index].specialSymbolId; //当前行id
        }
    }
};
</script>
