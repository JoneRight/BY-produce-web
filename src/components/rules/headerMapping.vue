    ...
    <div class="m15" style="height:calc(100% - 60px);overflow-y:hidden;">
        ...
        <div class="search-con">
            <Form :model="searchForm" ref="formInline" inline>
                ...
                <Button v-authorize="'RulesHeaderAdd'" type="primary" @click="addData()" style="float:right">+ 新增</Button>
            </Form>
        </div>
        
        <div class="bg-white mt15" style="height:100%;">
            <CustomTable v-authorize="'RulesHeaderList'" :columnConfig='dataCol' :columnDatas='dataList' :showModal="showModal" @onSort="sortColumn" @onOutput="showData" @onOk="showModal=false" @onCancel="showModal=false">
                <tr v-for="(item,index) in showDatas" :key="index">
                    <td v-for="(colData,colIndex) in dataCol" :key="colIndex" class="ellipsis">
                        <!-- {{item[colData.name]}} -->
                        <span v-if="colData.name=='op'">
                            <Button v-authorize="'RulesHeaderAudit'" v-if="item['state']==1" icon="md-checkmark-circle" size="small" title="通过" type="success" @click='ruleTongguo(index)'></Button>
                            <Button v-authorize="'RulesHeaderAudit'" v-if="item['state']==1" icon="md-close-circle" size="small" title="驳回" type="error" @click='ruleBohui(index)'></Button>
                            <Button v-authorize="'RulesHeaderDisable'" v-if="item['state']==2" icon="md-remove" size="small" title="停用" type="error" @click='ruleTingyong(index)'></Button>
                            <Button v-authorize="'RulesHeaderEnable'" v-if="item['state']==0" icon="md-play" size="small" title="启用" type="success" @click='ruleQiyong(index)'></Button>
                            <Button v-authorize="'RulesHeaderEdit'" icon="md-create" size="small" title="编辑" type="primary" @click='ruleBianji(index)'></Button>
                            <Button v-authorize="'RulesHeaderDel'" icon="md-close" size="small" title="删除" type="error" @click="ruleShanchu(index)"></Button>
                        </span>
                        <CheckboxGroup v-else-if="colData.name=='sel'" v-model="selectList" style="height:100%;">
                            <Checkbox size='small' :key="index" :value="JSON.stringify(item['headMappedId'])" @change="clickMe(index)"></Checkbox>
                        </CheckboxGroup>
                        ...
                        <span v-else-if="colData.name=='base_head_name'">
                            {{item['head_name']}}->{{item[colData.name]}}
                        </span>
                        <span v-else>
                            {{item[colData.name]}}
                        </span>
            ...
            <div style="" class="page-footer">
                <div class="mt15 text-center" v-if="dataList!=null && dataList.length>0">
                    ...
                    <Checkbox v-authorize="'RulesHeaderList'" size='small' label='全选' class="fl" value="1" falseValue='0' :checked="selectList.length==showDatas.length" @change="allClickMe"></Checkbox>
                    
                    <template v-if="oldStatus=='1'">
                        <Button v-authorize="'RulesHeaderAudit'" type="primary" @click="yesAuditBatch()" :disabled="selectList.length==0" class="inputCheck" style="left: 270px;margin-top: -10px;">通过</Button>
                        <Button v-authorize="'RulesHeaderAudit'" type="primary" @click="NoAuditBatch()" :disabled="selectList.length==0" class="inputCheck" style="left: 335px;margin-top: -10px;">驳回</Button>
                    </template>
                    <Button v-authorize="'RulesHeaderDelBatch'" type="primary" @click="deleteBatch()" :disabled="selectList.length==0" class="inputCheck" style="left: 400px;margin-top: -10px;">删除</Button>
        ...
        <Modal v-model="shanchuBatch" title="批量删除" width=420 @on-ok="doDeleteBatch()">
            您确定要批量删除选中的规则吗？
        </Modal>
...
export default {
        ...
        return {
            ...
            dataCol: [
                ...
                },
                {
                    ...
                    moveable: true,
                    render: params => {
                        return params.head_name + "->" + params.base_head_name;
                    }
                },
                {
                    text: "状态",
    ...
    methods: {
        ...
        getDataList(pageIndex) {
            ...
            Vue.ajax.send({
                ...
                data: {
                    ...
                    keywords: self.searchForm.keyword //规则
                },
        ...
        updateState(state) {
            var self = this;
            Vue.ajax.send({
                name: "headerMapping.updataState",
                data: {
                    headMappedId: self.commonId,
                    state: state
                },
                success: function(data) {
                    if (!!data && data.code == "0") {
        ...
        deleteBatch() {
            if (this.selectList.length < 1) {
                this.$Modal.error({
                    title: "错误",
                    closable: true,
                    content: "<h5>请选择需要批量删除的数据！</h5>"
                });
            } else {
                this.shanchuBatch = true;
            }
        },
        yesAuditBatch() {
            if (this.selectList.length < 1) {
        ...
        doDeleteBatch(state) {
            if (this.selectList.length < 1) {
                ...
                this.selectList.forEach(element => {
                    idsList.push(element.headMappedId);
                });
                //this.updateAudit(this.selectList, state);
                var self = this;
                Vue.ajax.send({
                    name: "headerMapping.deleteBatch",
                    data: {
                        idsList: this.selectList
                    },
                    success: function(data) {
                        if (!!data && data.code == "0") {
        ...
        updateAudit(idsList, state) {
            var self = this;
            Vue.ajax.send({
                name: "headerMapping.updateAudit",
                data: {
                    idsList: idsList,
                    state: state,
        ...
        clickMe(index) {
            ...
            var dataIndex = this.selectList.indexOf(
                this.showDatas[index].headMappedId+""
            );
            //存在就删除，不存在就添加
            if (dataIndex > -1) {
                this.selectList.splice(dataIndex, 1);
            } else {
                this.selectList.push(this.showDatas[index].headMappedId);
            }
        },
        //全选、非全选
        allClickMe(value){
            if(value==1){
                this.selectList=this.showDatas.map(a=>JSON.stringify(a['headMappedId']));
            }
            else{
                this.selectList=[];
            }
        ...
        ruleTongguo(index) {
            this.commonId = this.showDatas[index].headMappedId;
            this.auditSingle(2);
        },
        ruleBohui(index) {
            this.buTongGuo = true;
        ...
        ruleBianji(index) {
            this.editData(this.showDatas[index].headMappedId);
        },
        ruleShanchu(index) {
            this.delRule = true;
            this.commonId = this.showDatas[index].headMappedId; //当前行id
        }
    }
};
</script>
